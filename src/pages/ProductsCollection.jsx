    ...
    <div className='flex flex-col sm:flex-row gap-1 sm:gap-10 pt-10 border-t'>
      ...
      <div className='min-w-60'>
        ...
        <div className={`border border-gray-300 pl-5 py-3 mt-6 ${!showFilter && 'hidden'} sm:block`}>
          ...
          <div className='flex flex-col gap-2 text-sm font-light text-gray-700'>
            <p className='flex gap-2'>
              <input className='w-3' type='checkbox' value={'Lipsticks'} onChange={toggleCategory}/> Lipsticks
            </p>
            <p className='flex gap-2'>
              <input className='w-3' type='checkbox' value={'Blushes'} onChange={toggleCategory}/> Blushes
            </p>
            <p className='flex gap-2'>
              <input className='w-3' type='checkbox' value={'Eyelashes'} onChange={toggleCategory}/> Eyelashes
            </p>
            <p className='flex gap-2'>
              <input className='w-3' type='checkbox' value={'Highlighters'} onChange={toggleCategory}/> Highlighters
            </p>
            <p className='flex gap-2'>
              <input className='w-3' type='checkbox' value={'Nail polishes'} onChange={toggleCategory}/> Nail polishes
            </p>
          </div>
        </div>
        {/* Subcategory */}
        <div className={`border border-gray-300 pl-5 py-3 my-5 ${showFilter ? '' : 'hidden'} sm:block`}>
          ...
          <div className='flex flex-col gap-2 text-sm font-light text-gray-700'>
            <p className='flex gap-2'>
              <input className='w-3' type='checkbox' value={'Purple'} onChange={toggleSubCategory}/> Brown
            </p>
            <p className='flex gap-2'>
      ...
      <div className='flex-1'>
        ...
        <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 gap-y-6'>
          {filterProducts.map((item,index) => (
            <ProductItem key={index} name={item.name} id={item._id} price={item.price} image={item.image} description={item.description}/>
          ))}
        </div>
